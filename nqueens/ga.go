...
import (
	...
	"github.com/julioguillermo/nqueens_opt/export"
	"github.com/julioguillermo/nqueens_opt/tool"
)
...
func (ga *GA) Info() string {
	dur := time.Since(ga.start)
	ga.dur = dur
	bf := ga.GetBestError()
	pro := 100.0 - float64(bf)*100.0/float64(ga.MaxErrors)
	return fmt.Sprintf(
		"%dx%d - %d => [%.2f%% %s] %d/%d",
		ga.GenSize,
		ga.GenSize,
...
func (ga *GA) Save() {
	export.Export(ga.GenSize, fmt.Sprintf("%d - %s", ga.Generation, tool.GetDurStr(ga.dur)), ga.Population[0].Genome)
}
